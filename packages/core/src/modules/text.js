...
export function getCellTextInfo(cell, renderCtx, sheetCtx, option, ctx) {
    ...
    const tb = normalizedCellAttr(cell, "tb");
    const tr = normalizedCellAttr(cell, "tr");
    let rt = normalizedCellAttr(cell, "rt");
    let isRotateUp = 1;
    if (_.isNil(rt)) {
        if (tr === "0") {
            rt = 0;
    ...
    if (tr === "3") {
        ...
        if (tb === "2" || isInline) {
            ...
            if (isInline) {
                while (i <= inlineStringArr.length) {
                    ...
                    const width = textWidth * Math.cos((rt * Math.PI) / 180) +
                        textHeight * Math.sin((rt * Math.PI) / 180);
                    const height = textWidth * Math.sin((rt * Math.PI) / 180) +
                        textHeight * Math.cos((rt * Math.PI) / 180);
            ...
            rt = Math.abs(rt);
            const rtPI = (rt * Math.PI) / 180;
            const textWidthAll = textWidth * Math.cos(rtPI) + textHeight * Math.sin(rtPI);
            const textHeightAll = textWidth * Math.sin(rtPI) + textHeight * Math.cos(rtPI);
            if (rt !== 0) {
