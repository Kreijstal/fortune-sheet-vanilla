...
export function syncOverlay(store, overlay, state) {
  ...
  if (calInfo.count) wrap.appendChild(statItem(`${formula.count}: ${calInfo.count}`, "60px"));
  if (calInfo.numberC && calInfo.sum) wrap.appendChild(statItem(`${formula.sum}: ${calInfo.sum}`));
  if (calInfo.numberC && calInfo.average) wrap.appendChild(statItem(`${formula.average}: ${calInfo.average}`));
  if (calInfo.numberC && calInfo.max) wrap.appendChild(statItem(`${formula.max}: ${calInfo.max}`));
  if (calInfo.numberC && calInfo.min) wrap.appendChild(statItem(`${formula.min}: ${calInfo.min}`));
...
function syncSelectionLayer(store, overlay) {
  ...
  const oldBoxs = overlay.cellArea.querySelector("#luckysheet-cell-selected-boxs");
