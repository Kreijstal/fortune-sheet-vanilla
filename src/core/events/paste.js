...
import {
  ...
  execFunctionGroup,
  functionCopy,
} from './../modules/formula.js';
import { getdatabyselection, getQKBorder } from './../modules/cell.js';
import { genarate, update } from './../modules/format.js';
import { normalizeSelection, selectionCache } from './../modules/selection.js';
import { getSheetIndex, isAllowEdit } from './../utils/index.js';
import { hasPartMC, isRealNum } from './../modules/validation.js';
import { getBorderInfoCompute } from './../modules/border.js';
import {
  expandRowsAndColumns,
  storeSheetParamALL,
} from './../modules/sheet.js';
import { jfrefreshgrid } from './../modules/refresh.js';
import { setRowHeight } from './../api/index.js';
import { CFSplitRange } from './../modules/index.js';
import clipboard from './../modules/clipboard.js';
import { setFormulaCellInfo } from './../modules/formulaHelper.js';
      ...
      for (let r = 0; r <= range.row[1] - range.row[0]; r += 1) {
        for (let c = 0; c <= range.column[1] - range.column[0]; c += 1) {
          const index = getSheetIndex(
            ctx,
            ctx.luckysheet_copy_save?.dataSheetId
          );
          if (
  ...
  if (ctx.currentSheetId === copySheetId) {
    for (let i = c_r1; i <= c_r2; i += 1) {
      for (let j = c_c1; j <= c_c2; j += 1) {
        ...
        delete ctx.luckysheetfile[getSheetIndex(ctx, ctx.currentSheetId)]
          .hyperlink?.[`${i}_${j}`];
      }
    }
    // 边框
  ...
  if (selectionCache.isPasteAction) {
    ...
    if (
      ...
      for (let r = copy_r1; r <= copy_r2; r += 1) {
        ...
        for (let c = copy_c1; c <= copy_c2; c += 1) {
          ...
          if (v == null && d[r]?.[c]?.ct?.t === 'inlineStr') {
          ...
          if (isInlineStr) {
            ...
            if (cpDataArr[r - copy_r1][c - copy_c1].trim() !== v.trim()) {
              isEqual = false;
              break;
            }
          }
    ...
    if (
      ...
      if (txtdata.indexOf('table') > -1) {
        ...
        const index = getSheetIndex(ctx, ctx.currentSheetId);
        if (index != null) {
          if (ctx.luckysheetfile[index].config == null) {
            ctx.luckysheetfile[index].config = {};
          }
          if (ctx.luckysheetfile[index].config.rowlen == null) {
          ...
          forEach(trList, (tr) => {
            ...
            forEach(tr.querySelectorAll('td'), (td) => {
              ...
              if (styles.border != null) td.style.border = styles.border;
