...
import { getFailureText, validateCellData } from './dataVerification.js';
import { genarate, update } from './format.js';
import {
  delFunctionGroup,
  execfunction,
  execFunctionGroup,
...
import { isRealNull, isRealNum, valueIsError } from './validation.js';
...
export function isAllSelectedCellsInStatus(ctx, attr, status) {
  ...
  if (!_.isEmpty(ctx.luckysheetCellUpdate)) {
    ...
    if (startContainer === endContainer) {
      return !_.isEmpty(startContainer.parentElement?.style[cssField]);
    }
    if (
