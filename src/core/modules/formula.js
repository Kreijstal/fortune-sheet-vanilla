...
import { getRangetxt, mergeMoveMain, setCellValue } from './cell.js';
import { error } from './validation.js';
import { moveToEnd } from './cursor.js';
import { locale } from './../locale/index.js';
import { colors } from './color.js';
import { colLocation, mousePosition, rowLocation } from './location.js';
import {
...
export function isFormula(value) {
  return (
    typeof value === 'string' && value.slice(0, 1) === '=' && value.length > 1
  );
}
...
export class FormulaCache {
  constructor() {
    ...
    this.parser.on(
      'callRangeValue',
      (startCellCoord, endCellCoord, options, done) => {
        const context = that.parser.context;
        const id =
...
function findrangeindex(ctx, v, vp) {
  ...
  // 增加元素输入
    ...
    if (p == null) {
      ...
      } else if (p != null && !p[0].startsWith('=') && n.startsWith('=')) {
...
export function getrangeseleciton() {
  ...
  if (
    ...
    const spans = anchorElement.querySelectorAll('span');
    let txt = (spans[spans.length - 1]?.innerText || '').trim();
    if (txt.length === 0 && spans.length > 1) {
      txt = (spans[spans.length - 2].innerText || '').trim();
      return spans[0];
    }
...
export function israngeseleciton(ctx, istooltip) {
  ...
  if (
    ...
    if (txt.length === 0 && spans.length > 1) {
      txt = (spans[spans.length - 2].innerText || '').trim();
      txt = (spans[spans.length - 2].innerText || '').trim();
      ctx.formulaCache.rangeSetValueTo = spans;
    }
