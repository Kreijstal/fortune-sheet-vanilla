...
import { generateRandomSheetName, getSheetIndex } from './../utils/index.js';
 ...
 * @param {any} // if action is from websocket
 * @param {boolean} [there will be a new sheetID
isPivotTable]
...
export function addSheet(
  ctx,
  settings,
  newSheetID = undefined, // if action is from websocket, there will be a new sheetID
  isPivotTable = false,
  sheetName = undefined,
  sheetData = undefined
) {
  ...
  if (!_.isNil(sheetData)) {
    ...
    ctx.luckysheetfile.forEach((sheet) => {
      sheet.order =
        sheet.order < sheetData.order ? sheet.order : sheet.order + 1;
      return sheet;
    });
  }
